...
function psPatchDefines($source, $values)
{
    ...
    foreach ($flat as $i => $token) {
        ...
        if (isset($found[$key])) throw new PrestashopSetupException('Duplicate managed define in settings.inc.php.');
        $found[$key] = true;
        $depth = 0;
        $start = $flat[$i+3][2] + 1;
        for ($j=$i+4; $j<count($flat); $j++) {
            ...
            if ($depth === 0 && ($text === ')' || $text === ',')) {
                $patches[] = array($start, $flat[$j][2]-$start, ' ' . var_export($values[$key], true));
                break;
            }
            if (in_array($text, array('(', '[', '{'), true)) $depth++;
            if (in_array($text, array(')', ']', '}'), true)) $depth--;
        }
        if ($j === count($flat)) throw new PrestashopSetupException('Cannot parse managed define in settings.inc.php.');
    }
    $append = '';
    foreach ($values as $key => $value) {
        if (!isset($found[$key])) $append .= "\ndefine(" . var_export($key, true) . ', ' . var_export($value, true) . ');';
    }
