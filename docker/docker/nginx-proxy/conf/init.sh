...
if [[ "${SETUP_ENABLE_PMA:-0}" == 1 ]]; then
    /usr/local/bin/pma-access.sh > /etc/nginx/setup-pma-access.conf
    cat /etc/nginx/templates/setup-pma.conf >> /etc/nginx/conf.d/sites_env.conf
fi
